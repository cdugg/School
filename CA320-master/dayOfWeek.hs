...
dayOfWeek :: Date -> Day
dayOfWeek d
   | numDays d `mod` 7 == 0 = Sunday
   | numDays d `mod` 7 == 1 = Monday
   | numDays d `mod` 7 == 2 = Tuesday
   | numDays d `mod` 7 == 3 = Wednesday
   | numDays d `mod` 7 == 4 = Thursday
   | numDays d `mod` 7 == 5 = Friday
   | otherwise = Saturday

numDays :: Date -> Int
numDays (day, month, year)
   | year > 1753 = sum(mLengths (year - 1)) + numDays (day, month, year - 1)
   | month == Jan = day
   | month == Feb = day + 31
